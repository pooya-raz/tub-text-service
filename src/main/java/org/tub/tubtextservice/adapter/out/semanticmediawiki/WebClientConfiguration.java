...
public class WebClientConfiguration {
    ...
    SemanticMediaWikiClient tubClient(final WebClient.Builder builder) {
        ...
        final var webClient = builder.baseUrl(tubProperties.apiUrl())
                .filter(logRequest())
                .filter(retryFilter(tubProperties))
                .clientConnector(new ReactorClientHttpConnector(client))
                .codecs(codecs -> codecs.defaultCodecs().maxInMemorySize(size))
                .build();
        final var factory = HttpServiceProxyFactory.builder(WebClientAdapter.forClient(webClient))
    ...
    private ExchangeFilterFunction retryFilter(TubProperties properties) {
        return (request, next) -> next.exchange(request)
                ...
                .retryWhen(Retry.backoff(properties.retryMaxAttempts(), properties.retryBackoffPeriod())
