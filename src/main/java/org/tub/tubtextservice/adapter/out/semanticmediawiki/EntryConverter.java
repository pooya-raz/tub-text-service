...
class EntryConverter {
    ...
    private TubEntry convertTitle(TitlePrintouts title, final TubPrintouts tubPrintouts) {
        ...
        final var translatorPrintouts = Optional.ofNullable(translator)
        ...
        return new TubEntry(
                titleName, titleOriginal, person, manuscripts, editions, commentaries, sortTimeStamp, titleType);
    }
    ...
    private Person getPerson(final AuthorPrintouts authorPrintouts) {
        return Optional.ofNullable(authorPrintouts)
                ...
                .map(personDeath ->
                        new Author(authorPrintouts.fullNameTransliterated().get(0), personDeath))
                .orElse(null);
    }

    private ArrayList<Manuscript> getManuscripts(
            final Map<String, ArrayList<ManuscriptPrintouts>> manuscriptPrintoutsMap, final String titleName) {
        return Optional.ofNullable(manuscriptPrintoutsMap.get(titleName))
                .map(printouts -> printouts.stream()
    ...
    private ArrayList<Edition> getEditions(
        ...
        return Optional.ofNullable(editionPrintoutsMap.get(titleName))
                .map(printouts ->
                        printouts.stream().map(this::convertEdition).collect(Collectors.toCollection(ArrayList::new)))
                .orElseGet(ArrayList::new);
    }
    ...
    private Commentary convertCommentary(final TitlePrintouts titlePrintouts, final TubPrintouts tubPrintouts) {
        final var authorKey = titlePrintouts.author().stream().findFirst().orElse(null);
        final var authorPrintouts = Optional.ofNullable(authorKey)
                .map(MediaWikiPageDetails::fulltext)
                .map(a -> tubPrintouts.authors().get(a))
                .orElse(null);
        ...
        final var title =
                titlePrintouts.titleTransliterated().stream().findFirst().orElse("");
        final var sortTimeStamp = getSortTimeStamp(authorPrintouts);
    ...
    private Edition convertEdition(EditionPrintouts editionPrintouts) {
        return Edition.builder()
                .titleTransliterated(editionPrintouts.titleTransliterated().stream()
                        .findFirst()
                        .orElse(null))
                ...
                .placeOfPublication(editionPrintouts.city().stream()
                        .findFirst()
                        .map(MediaWikiPageDetails::fulltext)
                        .orElse(null))
    ...
    private PersonDeath convertDate(AuthorPrintouts source) {
        ...
        final var gregorian = getYearFromMediaWikiDate(source.deathGregorian(), source.deathGregorianText());
    ...
    private String getYearFromMediaWikiDate(List<MediaWikiDate> gregorian, List<String> gregorianText) {
        var year = "";
